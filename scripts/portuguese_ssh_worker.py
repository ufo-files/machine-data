...
for record in manifest.get("items", []):
    ...
    seen.add(rel)
    rel_path = Path(rel)
    name = rel_path.name.casefold()
    if name.startswith("favicon") or name.startswith("apple-touch-icon"):
        continue
    ...
    if info.st_size <= 0 or info.st_size > max_bytes:
        continue
    if rel_path.suffix.lower() in {".jpg", ".jpeg", ".png", ".tif", ".tiff"} and info.st_size < 16384:
        continue
